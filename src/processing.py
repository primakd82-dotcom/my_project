...
def filter_by_state(my_list, my_state='EXECUTED') -> list:
    """ Функция возвращает новый список словарей, содержащий только те словари,
     у которых ключ state соответствует указанному значению. """
    new_list = []

    for item in my_list:
        if item.get('state') == my_state:
...
def sort_by_date(my_list, descending=True) -> list:
    """ Функция возвращает новый список,
    отсортированный по дате (date) на убывание """
    return sorted(my_list, key=lambda x: x.get('date'), reverse=descending)
